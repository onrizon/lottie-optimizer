...
function parseDoc(svg: string): Document | null {
  try {
  ...
  const walk = (el: Element) => {
    ...
    if (style) {
      for (const decl of style.split(';')) {
        ...
        if (val && COLOR_ATTRS.includes(prop)) {
...
export function applySVGColorOverrides(svg: string, overrides: Record<string, string>): string {
  ...
  const walk = (el: Element) => {
    for (const attr of COLOR_ATTRS) {
      ...
      if (v) {
        const hex = normalizeColor(v);
        if (hex && overrides[hex]) el.setAttribute(attr, overrides[hex]);
      }
    }
    const style = el.getAttribute('style');
    if (style) {
      const rebuilt = style.split(';').map(decl => {
        const colonIdx = decl.indexOf(':');
        if (colonIdx === -1) return decl;
        const prop = decl.slice(0, colonIdx).trim();
        const val = decl.slice(colonIdx + 1).trim();
        if (!val || !COLOR_ATTRS.includes(prop.toLowerCase())) return decl;
        const hex = normalizeColor(val);
        if (hex && overrides[hex]) return `${prop}:${overrides[hex]}`;
        return decl;
      }).join(';');
